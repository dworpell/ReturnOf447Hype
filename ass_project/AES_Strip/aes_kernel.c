...
#pragma HLS PIPELINE
		{
#pragma HLS UNROLL
			State[0][i] = sbox0[State[0][i]];
			State[1][i] = sbox1[State[1][i]];
			State[2][i] = sbox2[State[2][i]];
			State[3][i] = sbox3[State[3][i]];
		}
	}
#ifdef DEBUG
	print_state(State);
...
{
	uint32 temp=0;
	temp=apint_set_range(temp,31,24,sbox0[apint_get_range(word,31,24)]);
	temp=apint_set_range(temp,23,16,sbox1[apint_get_range(word,23,16)]);
	temp=apint_set_range(temp,15,8, sbox2[apint_get_range(word,15,8)]);
	temp=apint_set_range(temp,7,0,  sbox3[apint_get_range(word,7,0)]);
...
#pragma HLS INTERFACE s_axilite bundle=control port=return clock=axi_clock
	...
	multi_run: for (int run=0; run<BUFSIZE; run=run+4)
	{
#pragma HLS PIPELINE
		uint8 State[4][4];
	...
	#pragma HLS ARRAY_RESHAPE variable=State complete factor=4 dim=0
		...
		Outer: for (i=0; i<4; i++){
	#pragma HLS UNROLL
			uint32 temp=input[i+run];
			Inner: for (j=0; j<4; j++) {
				State[j][i] = apint_get_range(temp, 7, 0);
				temp=temp>>8;
				//printf("State[%d][%d]: %x\n", i, j, State[i][j]);
			/*uint32 tempin = input[i];
			State[0][i]=apint_get_range(tempin, 7,0);
			State[1][i]=apint_get_range(tempin, 15,8);
			State[2][i]=apint_get_range(tempin, 23,16);
			State[3][i]=apint_get_range(tempin, 31,24);*/
			}
		}
	...
	#pragma HLS PIPELINE
			//output_inner: for (j=0; j<4; j++) {
				...
				output[i+run]=apint_concatenate(apint_concatenate(State[3][i],State[2][i]),apint_concatenate(State[1][i],State[0][i]));
