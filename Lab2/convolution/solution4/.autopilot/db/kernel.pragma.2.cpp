...
# 1 "./kernel.h" 1
# 57 "./kernel.h"
void convolve_kernel (DATA_T bufw[48][48][(5)][(5)],
        DATA_T bufi[48][31*(1)+(5)-1][31*(1)+(5)-1],
        DATA_T bufo[48][31][31]);
# 43 "kernel.cpp" 2
...
void convolve_kernel (DATA_T bufw[48][48][(5)][(5)],
        DATA_T bufi[48][31*(1)+(5)-1][31*(1)+(5)-1],
        DATA_T bufo[48][31][31])
{_ssdm_SpecArrayDimSize(bufw, 48);_ssdm_SpecArrayDimSize(bufi, 48);_ssdm_SpecArrayDimSize(bufo, 48);
_ssdm_SpecArrayPartition( bufi, 1, "COMPLETE", 0, "");
# 59 "kernel.cpp"

_ssdm_SpecArrayPartition( bufw, 2, "COMPLETE", 0, "");
# 59 "kernel.cpp"

_ssdm_op_SpecInterface(0, "s_axilite", 0, 0, "", 0, 0, "control", "", "", 0, 0, 0, 0, "", "");
_ssdm_op_SpecInterface(bufw, "ap_memory", 0, 0, "", 0, 0, "", "", "", 0, 0, 0, 0, "", "");
_ssdm_op_SpecInterface(bufi, "ap_memory", 0, 0, "", 0, 0, "", "", "", 0, 0, 0, 0, "", "");
_ssdm_op_SpecInterface(bufo, "ap_memory", 0, 0, "", 0, 0, "", "", "", 0, 0, 0, 0, "", "");
 ...
 unsigned long to_b, ti_b, row_b, col_b;
  ...
  DATA_T temp_bufo=0;
  i_section:for(i=0;i<(5);i++)
  {
_ssdm_Unroll(0,0,0, "");
# 71 "kernel.cpp"

    j_section:for(j=0;j<(5);j++)
    {
   row_b_section:for(row_b=0;row_b<31;row_b++)
      {
  col_b_section:for(col_b=0;col_b<31;col_b++)
        {
    to_b_section:for(to_b=0;to_b<48;to_b++)
          {
...
# 79 "kernel.cpp"
   ...
   ti_b_section:for(ti_b=0;ti_b<48;ti_b++)
            {
             temp = temp + (bufw[to_b][ti_b][i][j] * bufi[ti_b][(1)*row_b+i][(1)*col_b+j]);
            }
